...
public class ChangeTripMode implements Provider<PlanStrategy> {
	...
	@Inject
	protected ChangeTripMode(ControlerConfigGroup controlerConfigGroup, GlobalConfigGroup globalConfigGroup,
	...
	public PlanStrategy get() {
		...
		if (controlerConfigGroup.getPlanCheckerLevel().equals(PlanCheckerLevel.AFTER_EACH_MODIFICATION)) {
			builder.addStrategyModule(new PlanChecker(globalConfigGroup, "Strategy ChangeTripMode after Step PlanSelector"));
		}
		builder.addStrategyModule(new TripsToLegsModule(tripRouterProvider, globalConfigGroup));
		if (controlerConfigGroup.getPlanCheckerLevel().equals(PlanCheckerLevel.AFTER_EACH_MODIFICATION)) {
			builder.addStrategyModule(new PlanChecker(globalConfigGroup, "Strategy ChangeTripMode after Step TripsToLegsModule"));
		}
		builder.addStrategyModule(new ChangeLegMode(globalConfigGroup, changeLegModeConfigGroup));
		if (controlerConfigGroup.getPlanCheckerLevel().equals(PlanCheckerLevel.AFTER_EACH_MODIFICATION)) {
			builder.addStrategyModule(new PlanChecker(globalConfigGroup, "Strategy ChangeTripMode after Step ChangeLegMode"));
		}
		...
		if (controlerConfigGroup.getPlanCheckerLevel().equals(PlanCheckerLevel.AFTER_EACH_MODIFICATION) ||
			...
			builder.addStrategyModule(new PlanChecker(globalConfigGroup, "Strategy ChangeTripMode after Step ReRoute"));
		}
